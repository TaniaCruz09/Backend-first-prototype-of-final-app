"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GruposConEstudiantesService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const utilities_1 = require("../../../common/helpers/utilities");
const gruposConEstudiantes_entity_1 = require("../entities/gruposConEstudiantes.entity");
let GruposConEstudiantesService = class GruposConEstudiantesService {
    constructor(grupoRepository) {
        this.grupoRepository = grupoRepository;
    }
    async createGrupo(createGrupoDto) {
        try {
            const nuevoGrupo = this.grupoRepository.create(createGrupoDto);
            return await this.grupoRepository.save(nuevoGrupo);
        }
        catch (error) {
            utilities_1.Utilities.catchError(error);
        }
    }
    async getGrupo() {
        try {
            const grupo = await this.grupoRepository.find({
                relations: ['grupo', 'estudiante']
            });
            return grupo;
        }
        catch (error) {
            utilities_1.Utilities.catchError(error);
        }
    }
    async getGrupoById(id) {
        try {
            const grupo = await this.grupoRepository.findOne({
                where: { id },
                relations: ['grupo', 'estudiante']
            });
            return grupo;
        }
        catch (error) {
            utilities_1.Utilities.catchError(error);
        }
    }
    async deleteGrupos(id) {
        try {
            const grupos = await this.grupoRepository.findOne({
                where: { id: id }
            });
            return await this.grupoRepository.remove(grupos);
        }
        catch (error) {
            utilities_1.Utilities.catchError(error);
        }
    }
    async updateGrupos(id, payload) {
        try {
            const grupos = await this.grupoRepository.preload({ id, ...payload });
            if (!grupos)
                throw new common_1.NotFoundException(`Grupos con ID ${id} no encontrado`);
            return await this.grupoRepository.save(grupos);
        }
        catch (error) {
            utilities_1.Utilities.catchError(error);
        }
    }
};
exports.GruposConEstudiantesService = GruposConEstudiantesService;
exports.GruposConEstudiantesService = GruposConEstudiantesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(gruposConEstudiantes_entity_1.GruposConEstudiantes)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], GruposConEstudiantesService);
//# sourceMappingURL=gruposConEstudiantes.service.js.map